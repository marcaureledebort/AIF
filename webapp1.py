...
path_http='http://annoy-db:5000'
def reco_webapp(genre,year):
    rep=requests.post(path_http+'/reco',json={'genre':genre,'year':year})
...
def reco_plot_webapp(title):
    rep=requests.post(path_http+'/reco_plot',json={'title':title})
...
def reco_poster_webapp(numero):
    response = requests.post(path_http+'/reco_poster', json={"numero":numero}).json()
...
def reco_poster_webapp_bis(image):
    ...
    response = requests.post(path_http+"/reco_poster_bis", files=files).json()
    #response = requests.post("http://annoy-db:5000/reco_poster", json={"numero":numero}).json()
    return [Image.open(i) for i in response['paths']]
...
def process_text(text,method,number_neighbor):
    ...
    if method=="Bag of Words":
        response=requests.post(path_http+'/text_reco',json={'text': text})
    else:
        response=requests.post(path_http+"/text_reco_glove",json={'text': text})
...
if __name__=='__main__':
    response = requests.get(path_http+'/infos')
    ...
    gr.TabbedInterface([i1,i2,i3,i4,demo],["Recommandation par score IMdB","Recommandation par poster","Recommandation par plot",'Recommandation par plot',"Reco new"]).launch(server_name='0.0.0.0')